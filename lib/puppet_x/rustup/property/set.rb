...
class PuppetX::Rustup::Property::Set < Puppet::Property
  ...
  #     }
  #
  # Suppose that `bar` actually has `a`, `b`, and `c` entries on the system. If
  # this method returns `true`, then the above resource will not be considered
  # changed. If it returns `false`, then it will be.
  #
  ...
  # entry within in the set. If two entries have the same identity, then it is
  # an error.
  ...
  # there is no corresponding entry in `is`, then it will _not_ be considered a
  # change.
  ...
  def insync?(is)
    ...
    if ignore_removed_entries
      # Every entry in `should` corresponds to an entry in `is`, so nothing was
      # added.
      true
    else
